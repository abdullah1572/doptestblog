...
import MintCollectionNft from '@/hooks/dataSender/mintCollectionNft';
import MintCollectionNftInfo from '@/hooks/dataFetcher/mintCollectionNftInfo';
...
const Collectiondetailpage = () => {
  ...
  const { mintCollectionNfts } = MintCollectionNft()
  const { mintCollectionNftsInfo } = MintCollectionNftInfo()
  ...
  async function mintNFtFunc() {
    let mintedNfts = await mintCollectionNftsInfo(dataset?.contractAddress)
    console.log(mintedNfts);
  
    if (parseFloat(mintedNfts?.totalLimit ) < parseFloat(mintedNfts?.minted) + count) {
    ...
    setLoader(true)
    try {
      let res = await mintCollectionNfts(dataset?.contractAddress, tokenIds?.length)
      if (res) {
        const res2 = axios
          .patch(`${api_url}/nfts/mint-collection`, {
        ...
        <div className="launchpaddetail-container">
          ...
          <div className="launchpaddetailinner">
            ...
            <div className="mintstages">
              <h6 className="minstageshead">Mint</h6>
              ...
              <div className="upcomingbottom">
                <div className="priceprogressmain">
                  ...
                  <div className="progressupper w-100 priceprogressmain-coll">
  ...
  <a className='minus-sign' onClick={handleIncrement}>
    <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 25 25" fill="none">
      ...
      <path d="M12.5 19.25C12.09 19.25 11.75 18.91 11.75 18.5V6.5C11.75 6.09 12.09 5.75 12.5 5.75C12.91 5.75 13.25 6.09 13.25 6.5V18.5C13.25 18.91 12.91 19.25 12.5 19.25Z" fill="white" />
               ...
               <>
                  <button disabled={parseFloat(dataset?.minted) >= parseFloat(dataset?.totalSupply)} onClick={mintNFtFunc} className={parseFloat(dataset?.minted) >= parseFloat(dataset?.totalSupply) ? "bluebtnexplore disable" :  "bluebtnexplore"}>Mint</button>
